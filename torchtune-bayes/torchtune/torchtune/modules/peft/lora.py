...
class LoRALinear(nn.Module, AdapterModule):
    ...
    def __init__(
        ...
        **quantization_kwargs,
        ...
        self.det_mode = False # whether or not to generate in the deterministic mode
        self.prune_pctg = 0 # percent of embedding dimensions to prune
        ...
        self.lora_a_mutheta = nn.Linear(in_features=in_dim, out_features=rank, bias=False)
        self.lora_b_mutheta = nn.Linear(in_features=rank, out_features=out_dim, bias=False)
        self.lora_a_logvartheta = nn.Linear(in_features=in_dim, out_features=rank, bias=False)
        self.lora_b_logvartheta = nn.Linear(in_features=rank, out_features=out_dim, bias=False)
        self.lora_a_mui = nn.Linear(in_features=in_dim, out_features=rank, bias=False)
        self.lora_b_mui = nn.Linear(in_features=rank, out_features=out_dim, bias=False)
        self.lora_a_logvari = nn.Linear(in_features=in_dim, out_features=rank, bias=False)
        self.lora_b_logvari = nn.Linear(in_features=rank, out_features=out_dim, bias=False)
        self.merged = False
        self.initialize_parameters()
        self.explicit_forward = True
        # Transient activation tensors populated during forward(); registered as
        # non-persistent buffers so model.to(device) moves them automatically,
        # but they are excluded from state_dict() checkpoints.
        self.register_buffer('phi_i', None, persistent=False)
        self.register_buffer('mu_i', None, persistent=False)
        self.register_buffer('logvar_i', None, persistent=False)
        self.register_buffer('mu_theta', None, persistent=False)
        self.register_buffer('logvar_theta', None, persistent=False)

    def to_empty(
        self, *, device: Optional[Union[str, torch.device, int]], recurse: bool = True
    ):
        self.lora_a_mutheta.to_empty(device=device, recurse=recurse)
        self.lora_a_mui.to_empty(device=device, recurse=recurse)
        self.lora_a_logvartheta.to_empty(device=device, recurse=recurse)
        self.lora_a_logvari.to_empty(device=device, recurse=recurse)
        self.lora_b_mutheta.to_empty(device=device, recurse=recurse)
        self.lora_b_mui.to_empty(device=device, recurse=recurse)
        self.lora_b_logvartheta.to_empty(device=device, recurse=recurse)
        self.lora_b_logvari.to_empty(device=device, recurse=recurse)

    def initialize_parameters(self):
        # Initialize as in
        # https://github.com/microsoft/LoRA/blob/4c0333854cb905966f8cc4e9a74068c1e507c7b7/loralib/layers.py#L119
        _lora_a_init_params(self.lora_a_mutheta)
        _lora_a_init_params(self.lora_a_mui)
        _lora_a_init_params(self.lora_a_logvartheta)
        _lora_a_init_params(self.lora_a_logvari)
        _lora_b_init_params(self.lora_b_mutheta)
        _lora_b_init_params(self.lora_b_mui)
        _lora_b_init_params(self.lora_b_logvartheta)
        _lora_b_init_params(self.lora_b_logvari)
    ...
    def adapter_params(self) -> List[str]:
        ...
        adapter_params = ["lora_a_mutheta.weight", 
                          "lora_b_mutheta.weight",
                          "lora_a_logvartheta.weight", 
                          "lora_b_logvartheta.weight",
                          "lora_a_mui.weight", 
                          "lora_b_mui.weight",
                          "lora_a_logvari.weight", 
                          "lora_b_logvari.weight"]
        return adapter_params
    
    def reparameterize(self, mean, logvar):
        eps = torch.normal(torch.zeros_like(mean), torch.zeros_like(logvar)+1)
        return eps * torch.exp(logvar * .5) + mean
    
    def prune(self, x):

        sorted_idx = torch.argsort(torch.abs(x), dim=-1)
        split_idx = int(x.shape[-1] * self.prune_pctg)
        sorted_idx = sorted_idx[..., :split_idx]
        #log.info(f'SORTED IDX {sorted_idx}')
        x.scatter_(dim=-1, index=sorted_idx, src=torch.zeros_like(sorted_idx, dtype=x.dtype, device='cuda'))
        #log.info(f'x {x}')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        lora_out_mui = self.lora_a_mui(self.dropout(x))
        lora_out_mui = (self.alpha / self.rank) * self.lora_b_mui(lora_out_mui)

        lora_out_logvari = self.lora_a_logvari(self.dropout(x))
        lora_out_logvari = (self.alpha / self.rank) * self.lora_b_logvari(lora_out_logvari) -20

        lora_out_mutheta = self.lora_a_mutheta(self.dropout(x))
        lora_out_mutheta = (self.alpha / self.rank) * self.lora_b_mutheta(lora_out_mutheta)
        lora_out_logvartheta = self.lora_a_logvartheta(self.dropout(x))
        lora_out_logvartheta = (self.alpha / self.rank) * self.lora_b_logvartheta(lora_out_logvartheta) -20

        if not self.det_mode:
            lora_out = self.reparameterize(lora_out_mui, lora_out_logvari)
        else:
            lora_out = lora_out_mui
        if self.explicit_forward:
            self.phi_i = lora_out
            self.mu_i = lora_out_mui
            self.logvar_i = lora_out_logvari
            self.mu_theta = lora_out_mutheta
            self.logvar_theta = lora_out_logvartheta

        output = out + lora_out

        if self.prune_pctg != 0:

            output = self.prune(output)
